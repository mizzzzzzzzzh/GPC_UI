...
async def consumer(graph, window_size, ss):
        
    if ss not in st.session_state:
        st.session_state[ss] = defaultdict(partial(deque, [0]*window_size, maxlen=window_size))
    ...
    while True:
        with graph:
            ...
            st.session_state[ss]['y1'].append(new_num_y1)
            st.session_state[ss]['y2'].append(new_num_y2)
            i = 0
            x = np.arange(i, i+window_size)
            y1 = np.array(st.session_state.win1['y1'])
            y2 = np.array(st.session_state.win1['y2'])
...
async def main():
    await asyncio.gather(
        ...
        metric(),
        consumer(c11, 30, "win1"),
        consumer(c12, 30, "win2"),
        consumer(c21, 30, "win3"),
        consumer(c22, 30, "win4"),
        tables(c31, ctl_df),
